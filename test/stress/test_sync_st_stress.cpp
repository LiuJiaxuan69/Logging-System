...
TEST(sync_logger_stress_st)
{
    ...
    builder->buildLoggerName("sync_st");
    ...
    {
        ScopedTimer timer("sync_st", std::cout, &elapsed_ms);
        for (int i = 0; i < N; ++i)
        {
            LOGINFO(lg, "{}", inf);
        }
    ...
    std::cout << "[RESULT] sync logger single thread: " << N << " logs of "
